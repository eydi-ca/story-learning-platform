  ...
  return (
    <section className="space-y-6">
      ...
      <div className="admin-panel p-6">
        <h2 className="text-2xl font-semibold tracking-tight text-slate-900">Chapter Summary</h2>
        <p className="mt-2 text-sm leading-6 text-slate-600">Shows which chapters students complete most often and which chapters may need review.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {chapters.map((chapter) => {
            const records = progress.filter((item) => item.chapterId === chapter.id)
            return <div className="admin-card p-4" key={chapter.id}><p className="font-medium text-slate-900">{chapter.title}</p><p className="mt-1 text-sm text-slate-600">{records.length} attempts - {records.filter((item) => item.passed).length} passes</p></div>
          })}
        </div>
      </div>
    </section>
...
function Report({ title, body }) {
  return <div className="admin-panel p-6"><h2 className="text-xl font-semibold tracking-tight text-slate-900">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{body}</p></div>
}
